...
import { datasetService } from '../src/services/api';
...
const SmartImputationPanel = ({ storageUrl, onImputationSuccess }) => {
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-indigo-500/20 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950/20 shadow-xl shadow-indigo-950/20 space-y-6">
      ...
      {error && (
        <div className="p-4 bg-rose-500/10 border border-rose-500/20 text-rose-455 rounded-xl text-xs font-semibold flex items-center gap-2.5">
          <AlertCircle size={15} className="flex-shrink-0 text-rose-455" />
          <span>{error}</span>
        </div>
      )}
